...
class _面板:
    ...
    def getallpos(self):# 将散点给到自己的三个顶点
        centerofball=root(lambda x:np.array([distance(self.ds[0].pos,x)-R,distance(self.ds[1].pos,x)-R,distance(self.ds[2].pos,x)-R]),[0,0,0]).x
        self.ds[0].boarddots[self.ind]=(getallpos2ball(_getallpos(self.ds[0].pos,self.ds[1].pos,self.ds[2].pos),centerofball))
        self.ds[1].boarddots[self.ind]=(getallpos2ball(_getallpos(self.ds[1].pos,self.ds[2].pos,self.ds[0].pos),centerofball))
        self.ds[2].boarddots[self.ind]=(getallpos2ball(_getallpos(self.ds[2].pos,self.ds[0].pos,self.ds[1].pos),centerofball))
    def refarea(self):
        centerofball=root(lambda x:np.array([distance(self.ds[0].pos,x)-R,distance(self.ds[1].pos,x)-R,distance(self.ds[2].pos,x)-R]),[0,0,0]).x
        percent = _refdots(
